...
const state = {
  lessons: JSON.parse(JSON.stringify(initialLessons)),
...
const statusCountElements = {
  pending: document.querySelector('[data-count-status="pending"]'),
  revision_requested: document.querySelector(
    '[data-count-status="revision_requested"]'
  ),
  approved: document.querySelector('[data-count-status="approved"]'),
...
function updateMetrics() {
  ...
  lessons.forEach((lesson) => {
    if (counts[lesson.status] !== undefined) {
      counts[lesson.status] += 1;
    }
  });

  Object.entries(statusCountElements).forEach(([status, element]) => {
    if (element) {
...
function applyFilters(lesson) {
  const { cefr, theme, lessonType, grammar } = state.filters;

  const matchesStatus =
    state.activeStatus === "all" || lesson.status === state.activeStatus;
  const matchesCefr = cefr === "all" || lesson.tags.CEFR_LEVEL === cefr;
  const matchesTheme =
    theme === "all" || lesson.tags.theme_category === theme;
  const matchesLessonType =
    lessonType === "all" || lesson.tags.lesson_type === lessonType;
  const matchesGrammar =
    grammar.trim() === "" ||
    lesson.tags.grammer_focus
      .toLowerCase()
      .includes(grammar.trim().toLowerCase());
...
function createTagElement(key, value) {
  ...
  tag.append(label, document.createTextNode(` ${value}`));
  return tag;
}
...
function createLessonCard(lesson) {
  ...
  statusPill.className = `lesson-status status-${lesson.status}`;
  statusPill.textContent = statusMap[lesson.status] ?? lesson.status;
  ...
  meta.innerHTML = `
      <span>레슨 타입: ${lessonTypeMap[lesson.lessonType] ?? "N/A"}</span>
      <span>업데이트: ${formatDate(lesson.updatedAt)}</span>
      <span>생성: ${lesson.submittedBy}</span>
    `;
  ...
  Object.entries(lesson.tags).forEach(([key, value]) => {
    tagGroup.append(createTagElement(key, value));
  });
  const extraTags = Array.isArray(lesson.extraTags) ? lesson.extraTags : [];
  extraTags.forEach(({ key, value }) => {
    tagGroup.append(createTagElement(key, value));
  });

  card.append(header, meta, summary, tagGroup);

  const riskFlags = Array.isArray(lesson.riskFlags) ? lesson.riskFlags : [];
  if (riskFlags.length > 0) {
    const riskList = document.createElement("ul");
    riskList.className = "lesson-flags";
  ...
  notes.textContent =
    lesson.reviewerNotes?.trim() === ""
      ? "검수 메모가 아직 없습니다."
      : `최근 메모: ${lesson.reviewerNotes}`;

  const actions = document.createElement("div");
  actions.className = "action-group";

  const detailBtn = document.createElement("button");
  detailBtn.className = "btn-neutral";
  detailBtn.type = "button";
  detailBtn.textContent = "자세히 보기";
  detailBtn.addEventListener("click", () => openDialog(lesson.id));

  const approveBtn = document.createElement("button");
  approveBtn.className = "btn-primary";
  approveBtn.type = "button";
  approveBtn.textContent = "빠른 승인";
  ...
  revisionBtn.className = "btn-secondary";
  revisionBtn.type = "button";
  revisionBtn.textContent = "수정 요청";
  ...
  actions.append(detailBtn, approveBtn, revisionBtn, rejectBtn);
  footer.append(notes, actions);
  card.append(footer);
...
function renderLessons() {
  ...
  const filtered = state.lessons.filter(applyFilters);
  ...
  if (filtered.length === 0) {
    ...
    empty.innerHTML = `
        <header>
          <h2 class="lesson-title">검색 결과가 없습니다</h2>
        </header>
        <p class="lesson-summary">
          선택한 필터에 해당하는 수업이 없습니다. 필터를 조정하거나 초기화해 주세요.
        </p>
      `;
  ...
  filtered
    ...
    .forEach((lesson) => {
      lessonList.append(createLessonCard(lesson));
    });
}
...
const dialogFlagsWrapper = document.querySelector("#dialog-flags-wrapper");
const dialogFlags = document.querySelector("#dialog-flags");
const dialogNotes = document.querySelector("#dialog-notes");

function openDialog(lessonId) {
  const lesson = state.lessons.find(({ id }) => id === lessonId);
  if (!lesson) return;

  state.activeLessonId = lessonId;
  dialogTitle.textContent = lesson.title;
  dialogSubtitle.textContent = `CEFR ${lesson.tags.CEFR_LEVEL} · ${
    lessonTypeMap[lesson.lessonType]
  } · ${statusMap[lesson.status] ?? lesson.status} · ${formatDate(
    lesson.updatedAt
  )}`;
  dialogSummary.textContent = lesson.summary;

  dialogTags.innerHTML = "";
  Object.entries(lesson.tags).forEach(([key, value]) => {
    dialogTags.append(createTagElement(key, value));
  });
  const extraTags = Array.isArray(lesson.extraTags) ? lesson.extraTags : [];
  extraTags.forEach(({ key, value }) => {
    dialogTags.append(createTagElement(key, value));
  ...
  contentBlocks.forEach(({ heading, body = [] }) => {
    const block = document.createElement("section");
    block.className = "lesson-preview-section";

    if (heading) {
      const headingEl = document.createElement("h4");
      headingEl.className = "lesson-preview-title";
      headingEl.textContent = heading;
      block.append(headingEl);

      const divider = document.createElement("hr");
      divider.className = "lesson-preview-divider";
    ...
    dialogContent.append(block);
  });

  const riskFlags = Array.isArray(lesson.riskFlags) ? lesson.riskFlags : [];
  if (riskFlags.length > 0) {
    dialogFlagsWrapper.style.display = "block";
    dialogFlags.innerHTML = "";
    riskFlags.forEach((flag) => {
      const li = document.createElement("li");
      li.textContent = flag;
      dialogFlags.append(li);
    });
  } else {
    dialogFlagsWrapper.style.display = "none";
  }

  dialogNotes.value = lesson.reviewerNotes ?? "";

  if (typeof lessonDialog.showModal === "function") {
    lessonDialog.showModal();
  } else {
    alert("이 브라우저는 dialog 요소를 지원하지 않습니다.");
  }
}

function handleStatusChange(lessonId, nextStatus, notes = "") {
  const lesson = state.lessons.find(({ id }) => id === lessonId);
  if (!lesson) return;
  ...
  if (notes.trim() !== "") {
    lesson.reviewerNotes = notes;
  }

  updateMetrics();
  renderLessons();

  if (state.activeLessonId === lessonId && lessonDialog.open) {
    dialogNotes.value = lesson.reviewerNotes ?? "";
    const statusLabel = statusMap[nextStatus] ?? nextStatus;
    dialogSubtitle.textContent = `CEFR ${lesson.tags.CEFR_LEVEL} · ${
      lessonTypeMap[lesson.lessonType]
    } · ${statusLabel} · ${formatDate(lesson.updatedAt)}`;
  }
}

const filterControls = {
  cefr: document.querySelector("#filter-cefr"),
  theme: document.querySelector("#filter-theme"),
  lessonType: document.querySelector("#filter-lesson-type"),
  grammar: document.querySelector("#filter-grammar"),
};
...
function updateStatusTabs() {
  statusTabs.forEach((tab) => {
    ...
    tab.classList.toggle("status-tab--active", status === state.activeStatus);
  });
}
...
lessonDialog
  ...
  .addEventListener("submit", (event) => {
    ...
    const notes = dialogNotes.value;

    if (state.activeLessonId) {
      handleStatusChange(state.activeLessonId, action, notes);
    }
    lessonDialog.close(action);
  });

lessonDialog
  .querySelector(".dialog-close")
  .addEventListener("click", () => lessonDialog.close("close"));
